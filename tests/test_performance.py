...
from tests.constants import TestMonetaryConstants, TestUserData, TestValidationData


# === Фикстуры для PostgreSQL в контейнере ===


@pytest.fixture(scope="function")
def postgres_container():
    """Создает изолированный PostgreSQL контейнер для performance тестов."""
    client = docker.from_env()
    container = None

    # Останавливаем существующие контейнеры performance
    container_name = "balance-hub-postgres-performance"
    try:
        existing = client.containers.get(container_name)
        existing.stop(timeout=10)
        existing.remove()
        print(f"Удален существующий контейнер: {container_name}")
    except docker.errors.NotFound:
        pass
    except Exception as e:
        print(f"Предупреждение: Не удалось очистить существующий контейнер: {e}")

    try:
        # Запускаем новый изолированный контейнер
        container = client.containers.run(
            "postgres:16-alpine",
            name=container_name,
            environment={
                "POSTGRES_DB": "balance_hub_perf",
                "POSTGRES_USER": "perf_user",
                "POSTGRES_PASSWORD": "perf_pass",
            },
            ports={"5432/tcp": 5435},  # Используем порт 5435 для полной изоляции
            detach=True,
            remove=False,  # Не удаляем автоматически, контролируем сами
            restart_policy={"Name": "no"},
        )

        # Ждем готовности БД с проверкой
        max_attempts = 30
        for attempt in range(max_attempts):
            try:
                result = container.exec_run(
                    "pg_isready -h localhost -p 5432 -U perf_user -d balance_hub_perf"
                )
                if result.exit_code == 0:
                    print(f"PostgreSQL готов после {attempt + 1} попыток")
                    break
            except Exception as e:
                print(f"Попытка {attempt + 1}: {e}")

            if attempt == max_attempts - 1:
                raise RuntimeError("Контейнер PostgreSQL не удалось запустить")
        ...
        if container:
            try:
                # Получаем свежую ссылку на контейнер (может быть обновлена)
                container.reload()
                ...
                print("Удаляем контейнер...")
                container.remove(force=True)
                print("Контейнер успешно удален")

            except docker.errors.NotFound:
                print("Контейнер уже удален")
            except Exception as e:
                print(f"Предупреждение при очистке контейнера: {e}")
                # Попытка принудительного удаления через API
                try:
                    client.containers.get(container_name).remove(force=True)
                    print("Контейнер принудительно удален")
                except Exception:
                    print(
                        f"Очистка контейнера не удалась, может потребоваться ручное удаление: {container_name}"
                    )

        # Дополнительная проверка и очистка
        try:
            remaining = client.containers.get(container_name)
            remaining.remove(force=True)
            print("Очищен оставшийся контейнер")
        except docker.errors.NotFound:
            pass
        except Exception as e:
            print(f"Финальное предупреждение очистки: {e}")


@pytest.fixture(scope="function")
def performance_database_url(postgres_container):
    """URL для подключения к изолированному PostgreSQL контейнеру."""
    return "postgresql+asyncpg://perf_user:perf_pass@localhost:5435/balance_hub_perf"


@pytest.fixture(scope="function")
def performance_sync_database_url(postgres_container):
    """Синхронный URL для Alembic миграций."""
    return "postgresql+psycopg2://perf_user:perf_pass@localhost:5435/balance_hub_perf"


def run_alembic_migrations(database_url: str) -> None:
    """Применяет миграции Alembic к тестовой БД с правильной настройкой драйвера."""
    ...
    # Преобразуем асинхронный URL в синхронный для Alembic (правильный формат)
    sync_database_url = database_url.replace("postgresql+asyncpg", "postgresql")

    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".ini", delete=False
    ) as temp_file:
        # Читаем оригинальный alembic.ini
        with open(alembic_ini_path, "r", encoding="utf-8") as original:
            content = original.read()

        # Заменяем URL базы данных на синхронный
        content = content.replace(
            "sqlalchemy.url = driver://user:pass@localhost/dbname",
            f"sqlalchemy.url = {sync_database_url}",
        )

        # Убеждаемся что script_location правильный
        content = content.replace(
        ...
        temp_ini_path = temp_file.name

    try:
        # Устанавливаем переменные окружения для изоляции
        env = os.environ.copy()
        env["DATABASE_URL"] = sync_database_url  # Используем синхронный URL
        env["SYNC_DATABASE_URL"] = sync_database_url

        # Проверяем, что БД доступна
        try:
            # Для psycopg2 нужны отдельные параметры
            ...
            conn.close()
            print("Тест подключения к базе данных успешен")
        except Exception as e:
            print(f"Тест подключения к базе данных не удался: {e}")
            raise RuntimeError(f"Не удается подключиться к базе данных: {e}")

        # Запускаем миграции
        cmd = ["poetry", "run", "alembic", "-c", temp_ini_path, "upgrade", "head"]
        result = subprocess.run(
            cmd, cwd=project_root, env=env, capture_output=True, text=True, timeout=120
        )

        if result.returncode != 0:
            print(f"Миграция Alembic не удалась с кодом возврата: {result.returncode}")
            print(f"Stderr: {result.stderr}")
            print(f"Stdout: {result.stdout}")
            raise RuntimeError(f"Миграция Alembic не удалась: {result.stderr}")

        print("Миграции Alembic успешно применены")
        print(f"Вывод миграции: {result.stdout}")
...
@pytest.fixture(scope="function")
async def performance_sessionmaker(
    performance_database_url, performance_sync_database_url
):
    """Создает sessionmaker для performance тестов с применением миграций."""
    from app.db.base import Base
    from app.models import (  # noqa: F401 - импортируем модели для создания таблиц
        account,
        payment,
        user,
    )

    # Пытаемся применить миграции Alembic, но если не получается - создаем таблицы напрямую
    migration_success = False
    try:
        run_alembic_migrations(performance_sync_database_url)
        migration_success = True
        print("Миграции Alembic успешно применены")
    except Exception as e:
        print(f"Миграция Alembic не удалась: {e}")
        print("Переход к прямому созданию таблиц")

    # Создаем движок для тестов с оптимизированными настройками
    perf_engine = create_async_engine(
        performance_database_url,
        echo=False,
        future=True,
        pool_size=3,  # Минимальный размер пула
        max_overflow=7,  # Небольшой overflow
    ...
    # Если миграции не применились, создаем таблицы напрямую
    if not migration_success:
        try:
            async with perf_engine.begin() as conn:
                await conn.run_sync(Base.metadata.create_all)
            print("Таблицы базы данных успешно созданы через SQLAlchemy")
        except Exception as e:
...
async def performance_app(
    ...
    performance_database_url,
    performance_sync_database_url,
):
    """Создает полностью изолированное тестовое приложение с синглтон настройками."""
    from app.core.config import Settings, get_settings
    from app.db.session import get_db_session
    from app.main import create_app

    # Создаем ЕДИНЫЙ экземпляр настроек для всего теста (решение проблемы изоляции)
    _performance_settings_singleton = Settings(
        app_name="BalanceHub Performance Test",
        env="performance_test",
        debug=False,  # Отключаем debug для производительности
        cors_origins=performance_settings["cors_origins"],
        jwt_secret=performance_settings["jwt_secret"],
        jwt_algorithm=performance_settings["jwt_algorithm"],
        jwt_expires_minutes=60,
        webhook_secret_key=performance_settings["webhook_secret_key"],
        database_url=performance_database_url,
        sync_database_url=performance_sync_database_url,
    )

    # Создаем приложение
    app = create_app()

    # Переопределяем зависимость для БД с полной изоляцией
    async def override_get_db_session():
        async with performance_sessionmaker() as session:
            yield session

    # Переопределяем настройки с ЕДИНЫМ экземпляром (всегда один и тот же объект)
    def override_get_settings():
        return _performance_settings_singleton

    app.dependency_overrides[get_db_session] = override_get_db_session
    app.dependency_overrides[get_settings] = override_get_settings

    # Сохраняем ссылку на настройки в приложении для доступа в тестах
    app.state.performance_settings = _performance_settings_singleton
...
@pytest.fixture(scope="function")
def performance_app_settings(performance_app):
    """Возвращает синглтон настроек из performance приложения."""
    return performance_app.state.performance_settings


@pytest.fixture(scope="function")
def make_performance_token(performance_app_settings) -> callable:  # type: ignore[type-arg]
    """Создать функцию для генерации тестовых токенов для performance тестов."""
    from app.core.security import create_access_token

    def _make_token(user_id: int) -> str:
        """Создать токен для пользователя используя ЕДИНЫЕ настройки из приложения."""
        return create_access_token(
            user_id,
            performance_app_settings.jwt_secret,
            performance_app_settings.jwt_algorithm,
            performance_app_settings.jwt_expires_minutes,
        )
...
@pytest.fixture(scope="function")
def performance_settings() -> dict:
    """Настройки для performance тестов."""
    from tests.constants import TestUserData
